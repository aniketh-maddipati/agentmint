...
impl JtiStore {
    ...
    pub fn check_and_insert(&self, jti: &str, exp: DateTime<Utc>) -> Result<()> {
    ...
    fn cleanup_expired_inner(entries: &mut HashMap<String, DateTime<Utc>>) {
        let now = Utc::now();
...
mod tests {
    ...
    fn expired_entries_cleaned_up() -> Result<()> {
        ...
        store.check_and_insert("jti-new", future)?;
