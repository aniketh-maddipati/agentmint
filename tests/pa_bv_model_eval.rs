//! Optional live OpenAI BV eval — ignored by default so CI stays offline.
//! Run with:
...
use mint_run::lab::eval::{run_live_openai_eval, run_scripted_eval};
...
fn live_openai_eval_opt_in() {
    ...
    eprintln!(
        "live eval model={} overall={} scores={:?}",
        report.model_id, report.overall_passed, report.scores
    );
    assert!(report.live);
    assert!(report.overall_passed, "live model eval failed: {report:?}");
}
